...
async def get_users(db: AsyncSession, filter: UserFilter) -> list[dict]:
    query = select(User.id, User.name, User.login, User.department)
    if filter.id:
...
async def get_admins(db: AsyncSession) -> list[dict]:
    query = select(User.id, User.name, User.login, User.department).where(User.admin_pass.is_not(None))
    result = await db.execute(query)
    return [row._asdict() for row in result.all()]
